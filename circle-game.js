var cg = {
	lastTime: (new Date()).getTime(), config: {
     ...
     death: function() {
		...
		$.ajax({
			url: 'https://www.polkadotgame.com/scores.php',
			data: 'chk=1' + '&p=' + score, 
			dataType: 'script',
			type: 'post',
			success: function (data) {
				...
				if (res == 'true') {
					...
					$('.submit').click(function(event) {
  						...
  						if ((init == '') || (init == 's') || (init == 's') || (init == 's') || (init == 's') || (init == 's') || (init == 's') || (init == 's') || (init == 's') || (init == 's')) {
  							var init = 'NOO';
  						}
   ...
   dispText: function() {
     ...
     this.ctx.shadowOffsetX = 2;
     this.ctx.shadowOffsetY = 2;
     w = this.ctx.measureText(t = 'Click to Start').width
     this.ctx.fillText(t, (this.config.width - w)/2, cg.config.height / 2)
     this.ctx.font = 'italic 20px Georgia'
     this.ctx.fontStyle = 'italic'
     this.ctx.fillStyle = 'white'
